...
![图示3](图示3.png)
然而我引入len变量之后反而m不够大也会超时  
**逻辑错误死循环**  
利用走值法我发现，  
(target-1)%len这个值，target-1始终为固定值e.g.4-1=3，len开始为num e.g.len=num=7，但随着删除node减小会有一刻达到3，(target-1)%len即达到结果为0，而i在进入下一次循环之前已经通过i += 1变为1，所以if i == (target - 1)%len:再也不会满足不会再删除node，i不断增长，循环无法停止。  
**求助解决方法**
![图示4](图示4.png)
