...
class Solution(object):
    def sortedSquares(self, nums):
        ...
        while counter >= 0:
            ...
            #可以简化为比较-nums[slow]和nums[fast], 就可以知道谁的平方大, 这样每次循环只需要计算一次乘法算平方(谁的平方大算谁)
            #nums[slow]和nums[fast]可能情况：[-,+][-,0][-,-][0,0][0,+][+,+]
            #如果nums[slow]为负，nums[fast]为非负（即两者异号），此举为比较绝对值，谁大选谁，绝对值相等选择谁都可以，-nums[slow]>nums[fast]选-nums[slow]，-nums[slow]<nums[fast]选nums[fast]
            #如果nums[slow]和nums[fast]同负，-nums[slow]>nums[fast]，nums[slow]小于等于nums[fast]，nums[slow]的绝对值大于等于nums[fast]的绝对值，依然符合-nums[slow]>nums[fast]选-nums[slow]，绝对值相等选择谁都可以
            #如果nums[slow]和nums[fast]同非负，-nums[slow] <= nums[fast], nums[slow]小于等于nums[fast]，nums[slow]的绝对值小于等于nums[fast]的绝对值，依然符合-nums[slow] <= nums[fast]选nums[fast]，绝对值相等选择谁都可以
            #综上，-nums[slow]>nums[fast]选-nums[slow]，-nums[slow] <= nums[fast]选nums[fast]
